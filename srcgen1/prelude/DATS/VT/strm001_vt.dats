...
| ~
strmcon_vt_cons(x1, xs) => f1(x1, xs)
)
#impltmp
<x0><r0>
...
case+ xs of
| ~
strxcon_vt_cons(x1, xs) => f1(x1, xs)
)
//
(* ****** ****** *)
(* ****** ****** *)
...
test
then
loop(xs) else (g_free(xs);false))
end(*let*)//strxcon_vt_cons(x1,xs)
)(*case+*)//end-of-[ loop(  xs  ) ]
...
in//let
(
  strxcon_vt_cons(y1, auxmain(xs)))
end//let//end-[strxcon_vt_cons(x1,xs)]
)
...
let
val () = g_free(x1) in auxloop(!xs)
end
) (* end of [strxcom_vt_cons(x1,xs)] *)
)
...
(*
  val opt =
  (
    mapopt$fopr0<x0><y0>(x1))
*)
  val opt = filter$test1<x0>(x1)
in
(*
  case+ opt of
  | optn_vt_nil() =>
...
//
end // end of [strxcon_vt_cons(x1,xs)]
)
...
in//let
strxcon_vt_cons(y1,auxmain(i1,xs))
end//endof(strxcon_vt_cons(x1,xs))
...
{x0:vt}
gseq_strmize0
<strx_vt(x0)><x0> = strx_vt_strmize0<x0>(*xs*)
#impltmp
{x0:vt}
gseq_strmize0
<strq_vt(x0)><x0> = strq_vt_strmize0<x0>(*xs*)
//
(* ****** ****** *)
(* ****** ****** *)
//
(***********************************************************************)
(* end of [ATS3/XANADU_srcgen1_prelude_DATS_VT_strm001_vt.dats] *)
(***********************************************************************)
