...
strxcon_vt_cons(x0, xs) =>
let
val y0 = imap$fopr0<x0><y0>(i0, x0)
in//let
strxcon_vt_cons(y0, auxmain(i0+1, xs))
end // strxcon_vt_cons
) (* end of [auxmain(i0,xs)] *)
...
$llazy
(
g_free(xs); auxloop(i0, $eval(xs)))
//
and
auxloop
(
i0: nint,
xs: strxcon_vt(x0)): strxcon_vt(y0) =
(
//
case+ xs of
| ~
