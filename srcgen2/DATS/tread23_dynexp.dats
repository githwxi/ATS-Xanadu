...
val lvl0 = errvl(d3e1) in//let
d3exp_errck
( lvl0+1, d3exp(loc0, D3Elval(d3e1)) )
endlet // end of [d3exp_lval_errck(...)]
//
(* ****** ****** *)
//
fun
...
d3exp_errck
( lvl0+1
, d3exp(loc0,D3Eelazy(dsym,d3e1,d3es)))
...
val loc = d3p.lctn() in
(
  d3pat_argtp_errck(loc, d3p1, t2p2))
end (*let*) // end-of-[else]
...
(err=e00)
then (d3e) else
d3exp_lval_errck(d3e.lctn(), d3e1)
end (*let*) // end of [f0_lval(d3e,err)]
//
(* ****** ****** *)
//
fun
...
let
val loc = d3e.lctn() in
d3exp_xazgn_errck( loc, d3el, d3er )
end (*let*) // end-of-[else]
...
let
val loc = d3e.lctn() in
d3exp_xchng_errck( loc, d3el, d3er )
end (*let*) // end-of-[else]
...
let
val loc = d3e.lctn() in
d3exp_elazy_errck(loc,dsym,d3e1,d3es)
end (*let*) // end-of-[else]
...
let
val loc = d3e.lctn() in
d3exp_t2ped_errck( loc, d3e1, t2p2 )
end (*let*) // end-of-[else]
